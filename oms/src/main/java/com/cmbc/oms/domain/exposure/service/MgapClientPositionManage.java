...
//            logger.info("积存金头寸查询结果返回结果:code={}, {}", mgapPosResponse.getReturnCode().getType(), JSONObject.toJSONString(mgapPosResponse.getTotalAll()))
    ...
    public PositionDataResponse getPositionSummary(String fxSymbol) {
        ...
        try {
            ...
            // 3. 积存金头寸统计完成
            // 累加损益
            hedgePositionSummary.setProfitAndLoss(hedgePositionSummary.getProfitAndLoss().add(mgapPositionSummary.getProfitAndLoss() != null ? mgapPositionSummary.getProfitAndLoss() : BigDecimal.ZERO));
            // 累加净头寸
            hedgePositionSummary.setNetPosition(hedgePositionSummary.getNetPosition().add(mgapPositionSummary.getNetPosition() != null ? mgapPositionSummary.getNetPosition() : BigDecimal.ZERO));
            // 累加净金额
            hedgePositionSummary.setNetAmount(hedgePositionSummary.getNetAmount().add(mgapPositionSummary.getNetAmount() != null ? mgapPositionSummary.getNetAmount() : BigDecimal.ZERO));
            if (BigDecimal.ZERO.compareTo(hedgePositionSummary.getNetPosition()) != 0) {
                // 被除数不为0，净敞口金额固定转化为负数，再做计算，均价=净敞口金额/净敞口数量，保留两位小数
                hedgePositionSummary.setAvgPrice(hedgePositionSummary.getNetAmount().negate().divide(hedgePositionSummary.getNetPosition(), 2, BigDecimal.ROUND_HALF_UP));
            }
