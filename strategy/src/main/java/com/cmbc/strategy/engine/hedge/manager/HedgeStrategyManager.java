...
public class HedgeStrategyManager implements ExecutionReportListener, com.cmbc.oms.domain.facade.MgapPositionUpdateListener, com.cmbc.oms.domain.facade.QuantPositionUpdateListener {
    ...
    @Autowired
    @org.springframework.beans.factory.annotation.Qualifier("goldHedgeIoPool")
    private com.cmbc.common.util.concurrent.ShardingThreadPool goldHedgeIoPool;

    @Autowired
    @org.springframework.beans.factory.annotation.Qualifier("goldHedgeStatPool")
    private com.cmbc.common.util.concurrent.ShardingThreadPool goldHedgeStatPool;

    @Autowired
    @org.springframework.beans.factory.annotation.Qualifier("goldHedgeEventPool")
    private com.cmbc.common.util.concurrent.ShardingThreadPool hedgeStrategyEventPool;
        ...
        if (!CollectionUtils.isEmpty(runningInstances)) {
            for (HedgeStrategyInstance instance : runningInstances.values()) {
                if (instance.isRunning()) {
                    hedgeStrategyEventPool.execute(instance.getInstanceId(), () -> {
    ...
    public void startStrategy(HedgeStrategyRequest request) {
        ...
        // [新增校验]: 启动时判断单笔最大下单量要大于等于境内各个合约的合约乘数 (至少1手)
        if (config.getSymbolTimeSlices() != null) {
            for (com.cmbc.strategy.domain.model.config.SymbolTimeSlice slice : config.getSymbolTimeSlices()) {
                boolean isAbroad = com.cmbc.oms.constant.BaseConstants.DOMESTIC_TYPE_OUTER.equalsIgnoreCase(slice.getDomesticType());
                if (!isAbroad) {
                    java.math.BigDecimal maxOrderQty;
                    if (com.cmbc.strategy.constant.BusinessConstant.SH_FUTURES_EXCHANGE.equals(slice.getExchCode())) {
                        maxOrderQty = config.getFutureMaxOrderQty();
                        java.math.BigDecimal chaseMaxOrderQty = config.getChaseFutureMaxOrderQty();
                        if (chaseMaxOrderQty != null && chaseMaxOrderQty.compareTo(maxOrderQty) < 0) {
                            maxOrderQty = chaseMaxOrderQty;
                        }
                    } else {
                        maxOrderQty = config.getSpotMaxOrderQty();
                        java.math.BigDecimal chaseMaxOrderQty = config.getChaseSpotMaxOrderQty();
                        if (chaseMaxOrderQty != null && chaseMaxOrderQty.compareTo(maxOrderQty) < 0) {
                            maxOrderQty = chaseMaxOrderQty;
                        }
                    }

                    com.cmbc.oms.domain.order.model.ContractInfoBasic contractInfo = basicParamCacheManager.getContractInfo().get(slice.getSymbol());
                    if (contractInfo == null || contractInfo.getUnit() == null) {
                        throw new IllegalStateException("启动策略失败：无法从缓存中获取境内合约 " + slice.getSymbol() + " 的基础参数(含合约乘数)");
                    }
                    
                    java.math.BigDecimal unit = contractInfo.getUnit();
                    if (maxOrderQty == null || maxOrderQty.compareTo(unit) < 0) {
                        throw new IllegalStateException("启动策略失败：境内合约 " + slice.getSymbol() + " 单笔最大下单量(" + maxOrderQty + "g)不足1手(乘数:" + unit + "g)，禁止启动！");
                    }
                }
            }
        }

        goldHedgeIoPool.execute(request.getInstanceId(), () -> {
            // 1. 策略交互上下文初始化
            ...
            context.setGoldHedgeEventPool(hedgeStrategyEventPool);
